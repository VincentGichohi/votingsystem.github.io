...
def generate_otp():
    ...
    for i in range(r.randint(5, 8)):
        opt += str(r.randint(1, 9))
    return otp


def dashboard(request):
    user = request.user
    # check if the voter has been verified
    if user.voter.otp is None and user.voter.verified == False:
...
def resend_otp(request):
    ...
    if settings.SEND_OTP:
        if voter.otp_sent >= 3:
            error = True
            response = "You have requested OTP 3 times. You cannot do this again!. Please enter previously sent OTP!"
        else:
            phone = voter.phone
            ...
            try:
                ...
                if message_is_sent:   # otp was sent successfully
                    ...
                    # Limited to three so voters don't exhause OTP balance
...
def verify(request):
    ...
    return render(request, 'voting/voter/result.html')
...
def verify_otp(request):
    ...
    if request.method != 'POST':
        ...
        if otp is None:
            messages.error(request, "Please provide a valid otp")
        else:
            # Get user OTP
...
def preview_vote(request):
    if request.method != "POST":
        ...
        # We do not need a loop over CSRF TOKEN
        ...
        for position in positions:
            ...
            if position.max_vote > 1:
                ...
                if len(form_position) > max_vote:
                    ...
                    data = ""
                    for form_candidate_id in form.position:
                        try:
...
def submit_ballot(request):
    if request.method != "POST":
        messages.error(request, "Please browse the system")
        return redirect(reverse('show_ballot'))
    ...
    form = dict(request.POST)
    form.pop("csrftokenmiddlewaretoken", None)   # Pop csrf_token
    ...
    for position in positions:
        ...
        if position.max_vote > 1:
            ...
            try:
                ...
                messages.error(
                    request, "Please borrow the system properly" + str(e))
                return redirect(reverse('show_ballot'))

    # Count total number of records inserted
    # Check if viz-a-viz-a form count
